...
smb | `smb -action <shares\|ls\|cat\|upload\|rm\|mkdir\|mv\|push> -host <target> -username <user> [-password <pass>] [-hash <NT hash>] [-share <name>] [-path <path>] [-source <local file>]` | SMB2 file operations on remote shares — list, browse, read/write/delete, push local files for lateral tool transfer. NTLM auth + pass-the-hash. Cross-platform (T1021.002, T1550.002, T1570).
