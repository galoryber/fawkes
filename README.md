...
Fawkes is a golang based agent with cross-platform capabilities. It supports **Windows** (EXE, DLL, and shellcode payloads), **Linux** (ELF binaries and shared libraries), and **macOS** (Mach-O binaries for Intel and Apple Silicon). 46 commands are cross-platform, with 47 additional Windows-only commands, 1 Windows+macOS command (screenshot), 3 Unix-only commands, 6 Linux-only commands, and 2 macOS-only commands for a total of 106. Supports HTTP egress and TCP peer-to-peer (P2P) linking for internal pivoting.
...
ticket | `ticket -action forge -realm <DOMAIN> -username <user> -domain_sid <SID> -key <hex_key> [-key_type aes256\|aes128\|rc4] [-format kirbi\|ccache] [-spn <SPN>]` | Forge Kerberos tickets — Golden Ticket (TGT via krbtgt key) or Silver Ticket (TGS via service key). Supports AES256, AES128, RC4/NTLM. Outputs kirbi (Rubeus) or ccache (Linux). Cross-platform (T1558.001, T1558.002).
