...
encrypt | `encrypt -action <encrypt\|decrypt\|encrypt-files\|decrypt-files> -path <file_or_glob> [-key <base64key>] [-confirm SIMULATE] [-max_files 100]` | AES-256-GCM file encryption/decryption. encrypt-files: batch by glob with .fawkes ext + recovery key (ransomware sim T1486, safety gate required). decrypt-files: reverse batch. Cross-platform (T1560.001, T1486).
...
secure-delete | `secure-delete [-action delete\|wipe] -path <file_or_dir> [-passes <n>] [-confirm DESTROY]` | Securely delete files (random overwrite + removal). wipe: aggressive patterned destruction (zeros/ones/alternating/random, 7-pass default T1485, safety gate). Cross-platform (T1070.004, T1485).
...
sniff | `sniff [-action capture\|poison] [-interface eth0] [-duration 30] [-response_ip <IP>] [-protocols llmnr,nbtns,mdns]` | Network sniffing and LLMNR/NBT-NS/mDNS poisoning. `capture`: passive credential extraction (HTTP, FTP, NTLM, Kerberos). `poison`: active responder that answers multicast name queries with attacker IP for credential interception. Cross-platform capture; Linux poison (T1040, T1557.001).
...
ssh | `ssh [-action exec\|push] -host <target> -username <user> [-password <pass>] [-key_path <path>] [-command <cmd>] [-source <local> -destination <remote>]` | Execute commands or push files to remote hosts via SSH. Lateral movement + tool transfer. Cross-platform (T1021.004, T1570).
