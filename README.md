...
eventlog | `eventlog -action <list\|query\|clear\|info\|enable\|disable> [-channel <name\|unit\|path>] [-event_id <id>] [-filter <xpath\|keyword\|timewindow>] [-count <max>]` | **(Windows, Linux)** Manage system event logs. Windows: wevtapi.dll channels. Linux: journald units and syslog file queries. List sources, query events, clear/vacuum logs, get info. MITRE T1070.001, T1562.002.
...
service | `service -action <query\|start\|stop\|create\|delete\|list\|enable\|disable> -name <name> [-binpath <path>]` | **(Windows, Linux, macOS)** Manage services — Windows via SCM API, Linux via systemctl, macOS via launchctl. Query, list, start, stop, enable, disable. Create/delete Windows only (T1543.003, T1543.004, T1562.001).
