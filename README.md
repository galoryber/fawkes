...
Fawkes is a golang based agent with cross-platform capabilities. It supports **Windows** (EXE, DLL, and shellcode payloads), **Linux** (ELF binaries and shared libraries), and **macOS** (Mach-O binaries for Intel and Apple Silicon). 40 commands are cross-platform, with 47 additional Windows-only commands, 1 Windows+macOS command (screenshot), 4 Unix-only commands, 6 Linux-only commands, and 2 macOS-only commands for a total of 101. Supports HTTP egress and TCP peer-to-peer (P2P) linking for internal pivoting.
...
cred-harvest | `cred-harvest -action <shadow\|cloud\|configs\|all> [-user <username>]` | **(Unix only)** Harvest credentials from Linux/macOS: /etc/shadow hashes, cloud provider configs (AWS/GCP/Azure/K8s), and application credential files. MITRE T1552.001, T1552.004, T1003.008.
...
klist | `klist -action <list\|purge\|dump> [-server <filter>]` | Enumerate, dump, and purge Kerberos ticket cache. Windows: LSA API via secur32.dll. Linux/macOS: ccache file parsing. Cross-platform (T1558, T1550.003).
...
winrm | `winrm -host <target> -username <user> -password <pass> -command <cmd> [-shell cmd\|powershell]` | Execute commands on remote Windows hosts via WinRM with NTLM authentication. Supports cmd.exe and PowerShell. Cross-platform (T1021.006).
