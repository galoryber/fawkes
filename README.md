...
make-token | `make-token -username <user> -domain <domain> -password <pass>` | **(Windows only)** Create a token from credentials and impersonate it (LOGON32_LOGON_INTERACTIVE). Changes both local and network identity. Similar to `runas` but in the current process.
