...
credman | `credman [-action <list\|dump\|vault>] [-filter <pattern>]` | **(Windows + Linux)** Enumerate platform credential stores. Windows: Credential Manager + Vault (CredEnumerateW, vaultcli.dll). Linux: GNOME Keyring (secret-tool), KDE KWallet, NetworkManager WiFi/VPN passwords, GNOME Online Accounts. `list`=metadata, `dump`=with secrets, `vault`=Windows only. MITRE T1555.004, T1555.001.
...
schtask | `schtask -action <create\|query\|delete\|run\|list\|enable\|disable\|stop> -name <name> [-program <path>] [-trigger <type>] [-filter <substring>]` | **(Windows + Linux)** Manage scheduled tasks. Windows: Task Scheduler COM API. Linux: crontab, systemd timers, at jobs. Triggers: DAILY/WEEKLY/MONTHLY/ONSTART (cron), systemd (timer unit), at (one-shot). Cross-platform (T1053).
