...
sniff | `sniff [-interface eth0] [-duration 30] [-ports 21,53,80,88,445] [-promiscuous true] [-save_pcap true]` | Passive network sniffing for credential and DNS capture. Auto-extracts HTTP Basic Auth, FTP credentials, NTLM domain\user, Kerberos AS-REP/TGS-REP principals, DNS queries. PCAP download option. Windows (SIO_RCVALL), Linux (AF_PACKET + BPF), macOS (/dev/bpf). Cross-platform (T1040, T1558.004).
