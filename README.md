...
etw | `etw -action <sessions\|rules\|agents\|patch\|restore\|...> [-session_name <name>] [-provider <guid\|shorthand>]` | **(Windows, Linux, macOS)** Audit/telemetry subsystem manipulation. Windows: ETW sessions/providers stop/blind/enable/patch/restore. Linux: auditd rules, journald clear/rotate, syslog config, SIEM agent detection. macOS: unified logging, security agent detection. (T1562, T1070.002).
...
hashdump | `hashdump [-action dump\|auto-spray] [-targets <hosts>] [-format json]` | **(Windows, Linux, macOS)** Extract local account password hashes. Windows: NTLM hashes from SAM registry (requires SYSTEM). Linux: /etc/shadow hashes with hash-type identification (requires root). macOS: Directory Services PBKDF2 hashes from user plists (requires root). **auto-spray**: dump hashes then automatically spray them via cred-check against target hosts. MITRE T1003.002, T1003.008.
