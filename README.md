...
- **Body transforms:** Set the `body_transforms` build parameter to a comma-separated transform chain that modifies HTTP request/response bodies after encryption and base64 encoding. Makes C2 traffic resemble legitimate file downloads or API calls. Available transforms:
  - `base64` — double base64 encoding
  - `hex` — hex encoding
  - `gzip` — gzip compression (reduces payload size)
  - `xor:<hex_key>` — XOR with repeating key (e.g., `xor:DEADBEEF`)
  - `prepend:<hex_bytes>` — prepend fixed bytes
  - `append:<hex_bytes>` — append fixed bytes
  - `mask:<type>` — wrap in file header/footer (`png`, `gif`, `jpeg`, `pdf`). Traffic appears as image/document downloads.
  - `netbios` — NetBIOS-style byte encoding (blends with SMB traffic)
  - Example: `body_transforms=gzip,mask:png` compresses then wraps as PNG image.
  - **Note:** The C2 server must apply matching reverse transforms.
