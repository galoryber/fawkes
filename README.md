...
Fawkes is a golang based agent with cross-platform capabilities. It supports **Windows** (EXE, DLL, and shellcode payloads), **Linux** (ELF binaries and shared libraries), and **macOS** (Mach-O binaries for Intel and Apple Silicon). 105 commands are cross-platform, with 62 additional Windows-only commands, 1 Windows+Linux command (mem-scan), 5 Unix-only commands, 8 Linux-only commands, and 2 macOS-only commands for a total of 188. Supports HTTP egress and TCP peer-to-peer (P2P) linking for internal pivoting.
