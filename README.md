...
sniff | `sniff [-interface eth0] [-duration 30] [-ports 21,80,445] [-promiscuous true]` | Passive network sniffing for credential capture. Auto-extracts HTTP Basic Auth, FTP credentials, NTLM domain\user, Kerberos AS-REP/TGS-REP principals. Windows (SIO_RCVALL), Linux (AF_PACKET + BPF), macOS (/dev/bpf). Cross-platform (T1040, T1558.004).
