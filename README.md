...
vss | `vss -action <list\|create\|delete\|delete-all\|extract\|inhibit-recovery> [-volume C:\\] [-id <id>] [-confirm true]` | **(Windows only)** Manage Volume Shadow Copies and system recovery. delete-all and inhibit-recovery for ransomware emulation (T1490). Extract locked files (NTDS.dit, SAM). MITRE T1003.003, T1490.
