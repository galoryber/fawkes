...
make-token | `make-token -username <user> -domain <domain> -password <pass>` | **(Windows only)** Create a token from credentials for NETWORK authentication (like `runas /netonly`). Local operations still use your original token, but network operations (SMB, WMI) use the new credentials. Does NOT require admin privileges.
