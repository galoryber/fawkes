...
persist-enum | `persist-enum [-category <all\|registry\|startup\|winlogon\|ifeo\|appinit\|tasks\|services>]` | **(Windows only)** Read-only enumeration of common persistence mechanisms — registry Run keys, startup folders, Winlogon, IFEO, AppInit_DLLs, scheduled tasks, non-Microsoft services (T1547, T1053, T1543).
