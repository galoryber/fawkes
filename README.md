...
dcom | `dcom -action exec -host <target> -command <cmd> [-args <arguments>] [-object mmc20\|shellwindows\|shellbrowser\|wscript\|excel]` | **(Windows only)** Execute commands on remote hosts via DCOM lateral movement. Five objects: MMC20.Application, ShellWindows, ShellBrowserWindow, WScript.Shell (Run), Excel.Application (RegisterXLL/DDEInitiate). MITRE T1021.003.
