...
credential-prompt | `credential-prompt [-action prompt\|device-code] [-title "Authentication Required"] [-message "Enter your credentials..."] [-icon caution]` | Display native credential dialog or abuse OAuth device code flow. prompt: macOS AppleScript, Windows CredUI, Linux zenity/kdialog. device-code: Azure AD OAuth device code flow — generates user code, polls for token capture. Reports to Mythic credential vault. Cross-platform (T1056.002, T1621).
