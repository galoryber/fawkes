...
// Literally all borrowed (stolen) from the Merlin build go file
var debugInfo = false

// Build is the function Mythic calls to compile the Agent
func Build(msg structs.PayloadBuildMessage) (response structs.PayloadBuildResponse) {
	...
	for header, data := range headers {
		...
		default:
			// Do nothing the agent used with Mythic is not programmed to take extra headers at this time
	...
	cmd.Dir = filepath.Join("/", "Mythic", "fawkes")
...
func NewPayload() (structs.PayloadType, error) {
	payload := structs.PayloadType{
		...
		Description:                            "Fawkes agent largely ripped of from Merlin and Freyja",
