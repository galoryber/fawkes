...
func init() {
	agentstructs.AllPayloadData.Get("fawkes").AddCommand(agentstructs.Command{
		...
		Description:         "Container escape and K8s operations — enumerate breakout vectors, exploit Docker/cgroup/nsenter, and interact with Kubernetes API (RBAC privesc paths, nodes, secrets, pod exec) (T1611, T1610, T1613, T1552.007, T1069.003)",
		HelpString:          "container-escape -action <check|docker-sock|cgroup|nsenter|mount-host|k8s-enum|k8s-secrets|k8s-rbac|k8s-nodes|k8s-deploy|k8s-exec> [-command '<cmd>'] [-image alpine] [-path /dev/sda1|<namespace>|*]",
		Version:             3,
		...
		CommandParameters: []agentstructs.CommandParameter{
			...
			},
			{
				...
				CLIName:          "path",
				ParameterType:    agentstructs.COMMAND_PARAMETER_TYPE_STRING,
				Description:      "Block device path for mount-host; K8s namespace override for k8s-enum/k8s-secrets/k8s-deploy/k8s-exec; k8s-rbac accepts <ns>, comma-separated <ns1,ns2>, or '*' for every namespace",
				DefaultValue:     "",
				ParameterGroupInformation: []agentstructs.ParameterGroupInfo{
					{
						ParameterIsRequired: false,
