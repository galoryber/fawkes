...
func init() {
	agentstructs.AllPayloadData.Get("fawkes").AddCommand(agentstructs.Command{
		Name:                "lolbin",
		Description:         "LOLBin/GTFOBin proxy execution — execute payloads through legitimate system binaries to bypass application whitelisting and EDR. Windows: rundll32, msiexec, regsvcs, regasm, mshta, certutil. Linux: python, curl, wget, gcc, perl, ruby, node, awk. macOS: osascript, swift, open, python, curl.",
		...
		Version:             2,
		Author:              "@galoryber",
		MitreAttackMappings: []string{"T1218", "T1218.011", "T1218.007", "T1218.009", "T1218.005", "T1059", "T1059.002", "T1059.006", "T1059.007", "T1105", "T1027.004"},
		...
		CommandParameters: []agentstructs.CommandParameter{
			{
				...
				Choices:       []string{"rundll32", "msiexec", "regsvcs", "regasm", "mshta", "certutil", "python", "curl", "wget", "gcc", "perl", "ruby", "node", "awk", "osascript", "swift", "open"},
				Description:   "LOLBin/GTFOBin technique. Windows: rundll32, msiexec, regsvcs, regasm, mshta, certutil. Linux: python, curl, wget, gcc, perl, ruby, node, awk. macOS: osascript, swift, open, python, curl.",
