...
func init() {
	agentstructs.AllPayloadData.Get("fawkes").AddCommand(agentstructs.Command{
		Name:                "ldap-query",
		Description:         "Query Active Directory via LDAP. Supports preset queries (users, computers, groups, domain-admins, admins, SPNs, AS-REP roastable, disabled, GPOs, OUs, password-never-expires) and custom LDAP filters.",
		HelpString:          "ldap-query -action users -server 192.168.1.1\nldap-query -action admins -server dc01\nldap-query -action gpo -server dc01\nldap-query -action query -server dc01 -filter \"(servicePrincipalName=*MSSQLSvc*)\"",
		Version:             2,
		Author:              "@galoryber",
		...
		CommandParameters: []agentstructs.CommandParameter{
			{
				...
				Choices:          []string{"users", "computers", "groups", "domain-admins", "spns", "asrep", "admins", "disabled", "gpo", "ou", "password-never-expires", "dacl", "query"},
				DefaultValue:     "users",
