...
func (c *SetenvCommand) Execute(task structs.Task) structs.CommandResult {
	...
	if err := json.Unmarshal([]byte(task.Params), &args); err != nil {
		...
		if strings.HasPrefix(params, "unset ") {
			...
			if idx := strings.Index(rest, "="); idx >= 0 {
				...
				return errorResult("set requires NAME=VALUE format")
			}
		} else {
			return errorResult("could not parse arguments. Use JSON or 'set NAME=VALUE' / 'unset NAME'")
		}
	}
	...
	case "set":
		if err := os.Setenv(args.Name, args.Value); err != nil {
			return errorf("setting %s: %v", args.Name, err)
		}
		return successf("Set %s=%s", args.Name, args.Value)

	case "unset":
		if err := os.Unsetenv(args.Name); err != nil {
			return errorf("unsetting %s: %v", args.Name, err)
		}
		return successf("Unset %s", args.Name)

	default:
		return errorf("unknown action %q (use 'set' or 'unset')", args.Action)
	}
}
