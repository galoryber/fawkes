...
func (c *CpCommand) Execute(task structs.Task) structs.CommandResult {
	...
	if task.Params == "" {
		return errorResult("No parameters specified. Usage: cp <source> <destination>")
	}

	// Try to parse as JSON
	if err := json.Unmarshal([]byte(task.Params), &args); err != nil {
		return errorf("parsing parameters: %v. Usage: cp <source> <destination>", err)
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			return errorf("source file not found: %s", args.Source)
		}
		if os.IsPermission(err) {
			return errorf("access denied to source file %s — check privileges", args.Source)
		}
		return errorf("cannot open source file %s", args.Source)
	}
	...
	if err != nil {
		return errorf("cannot read source file metadata for %s", args.Source)
	}

	if sourceInfo.IsDir() {
		return errorf("%s is a directory, not a file", args.Source)
	}
	...
	if err != nil {
		if os.IsPermission(err) {
			return errorf("access denied writing to %s — check privileges", args.Destination)
		}
		if os.IsNotExist(err) {
			return errorf("destination directory does not exist for %s", args.Destination)
		}
		return errorf("cannot create destination file %s", args.Destination)
	}
	...
	if err != nil {
		destFile.Close()
		return errorf("write failed during copy (disk full or I/O error)")
	}

	// Close destination file explicitly to flush writes and catch errors
	if err := destFile.Close(); err != nil {
		return errorf("failed to finalize %s (disk full or I/O error)", args.Destination)
	}
