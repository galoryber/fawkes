...
// Supports both short (HKLM) and long (HKEY_LOCAL_MACHINE) forms.
...
// readValue reads a single registry value and formats it for display.
...
// parseRegWriteValue converts a string value to the appropriate registry type and writes it.
func parseRegWriteValue(key registry.Key, name, data, regType string) error {
	switch strings.ToUpper(regType) {
	case "REG_SZ":
		return key.SetStringValue(name, data)
	case "REG_EXPAND_SZ":
		return key.SetExpandStringValue(name, data)
	case "REG_DWORD":
		val, err := strconv.ParseUint(data, 10, 32)
		if err != nil {
			val, err = strconv.ParseUint(strings.TrimPrefix(data, "0x"), 16, 32)
			if err != nil {
				return fmt.Errorf("invalid DWORD value '%s' (use decimal or 0x hex)", data)
			}
		}
		return key.SetDWordValue(name, uint32(val))
	case "REG_QWORD":
		val, err := strconv.ParseUint(data, 10, 64)
		if err != nil {
			val, err = strconv.ParseUint(strings.TrimPrefix(data, "0x"), 16, 64)
			if err != nil {
				return fmt.Errorf("invalid QWORD value '%s' (use decimal or 0x hex)", data)
			}
		}
		return key.SetQWordValue(name, val)
	case "REG_BINARY":
	...
	default:
		return fmt.Errorf("unsupported registry type '%s' (use REG_SZ, REG_EXPAND_SZ, REG_DWORD, REG_QWORD, or REG_BINARY)", regType)
	}
}

// regSearchReadValue reads a registry value and returns it as a string for search matching.
