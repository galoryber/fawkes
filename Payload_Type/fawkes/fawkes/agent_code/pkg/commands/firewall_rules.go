...
// firewall_rules.go implements firewall rule operations: status, list, add,
// delete, and enable/disable. Core COM setup is in firewall.go.
...
func firewallAdd(args firewallArgs) structs.CommandResult {
	...
	resultSB.WriteString(fmt.Sprintf("Firewall rule added:\n"))
	resultSB.WriteString(fmt.Sprintf("  Name:      %s\n", args.Name))
	resultSB.WriteString(fmt.Sprintf("  Direction: %s\n", fwDirectionToString(dir)))
	resultSB.WriteString(fmt.Sprintf("  Action:    %s\n", fwActionToString(action)))
	resultSB.WriteString(fmt.Sprintf("  Protocol:  %s\n", fwProtocolToString(proto)))
	if args.Port != "" {
		resultSB.WriteString(fmt.Sprintf("  Port:      %s\n", args.Port))
	}
	if args.Program != "" {
		resultSB.WriteString(fmt.Sprintf("  Program:   %s\n", args.Program))
	}
	resultSB.WriteString("  Enabled:   true\n")
	resultSB.WriteString("  Profiles:  All\n")

	return successResult(resultSB.String())
}
...
func firewallEnableDisable(args firewallArgs, enable bool) structs.CommandResult {
	...
	return successf("%s firewall rule: %s", actionStr, args.Name)
}
