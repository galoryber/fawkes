...
func TestGuardSleepPagesLargeData(t *testing.T) {
	largeBlob := make([]byte, 8192)
	...
	guard := guardSleepPages(vault)
	if guard == nil {
		t.Fatal("guardSleepPages returned nil for large data")
	}

	unguardSleepPages(guard, vault)

	if !bytes.Equal(vault.agentBlob, origAgent) {
		t.Error("large agentBlob data mismatch after round-trip")
	}
}

func TestGuardSleepPagesDataZeroed(t *testing.T) {
	vault := &sleepVault{
		key:       []byte("0123456789abcdef0123456789abcdef"),
		agentBlob: []byte("sensitive-data-here"),
	}
	...
	if guard.data == nil {
		t.Fatal("guard.data should not be nil while guarded")
	}

	unguardSleepPages(guard, vault)

	if guard.data != nil {
		t.Error("guard.data should be nil after unguard (munmap'd)")
	}
}
