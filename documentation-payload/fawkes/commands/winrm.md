...
Execute commands on remote Windows hosts via WinRM (Windows Remote Management) with NTLM authentication. Supports both `cmd.exe` and PowerShell shells.

Uses the `masterzen/winrm` library for WinRM protocol operations (pure Go, CGO_ENABLED=0). Works cross-platform — agent running on any OS can execute commands on remote Windows hosts with WinRM enabled.
...
### cmd.exe
```
[*] WinRM vagrant@192.168.100.52:5985 (cmd)
[*] Command: whoami
[*] Exit Code: 0
------------------------------------------------------------
north\vagrant
